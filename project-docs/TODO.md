...
- [x] 実際の`.trf`ファイルを用いたインポート結果の検証 → 実データ(C_Main.trf/C_bu.trf)で解析・グラフ表示とも正常動作を確認済み
- [x] 不正ファイル（非TRF、破損ファイル）投入時のエラーハンドリング確認 → Playwrightで合成ファイルを検証。サイズ不足・署名不一致とも正しく拒否・エラー表示
- [x] 同名ファイル再アップロード時の上書き・旧ファイル削除の確認 → 検証中に不具合発見・修正
    - operatorによる上書き時、旧TRFファイルがStorageに孤立して残るRLS不具合を発見(`trf_delete`ポリシーがadministrator限定だったため)
    - `006_fix_trf_delete_policy.sql`でadministrator/operator両方に許可するよう修正し、再検証で解消を確認
- [x] ロールごとのアクセス制御確認（Administrator/Operator/Stageman）→ Phase 7で実施済み
- [ ] 測定一覧での複数選択・グローバル平滑化切替のパフォーマンス確認 → スキップ（登録件数が少ないため必要になった時点で対応）
