...
- [ ] マイク一覧画面（管理番号・メーカー・型番・状態・最終測定日・測定回数を表示）
- [ ] 一覧の検索機能（管理番号・メーカー・型番・状態・測定者・期間）
- [ ] マイク詳細画面（管理番号・メーカー・型番・シリアル・状態・備考 + 測定履歴一覧）
- [ ] マイク管理（追加・編集）画面（Administratorのみ）
- [ ] マイク論理削除機能（Administratorのみ、`deleted_at`更新）
